...
function Slider({title, min, max}: SliderType) {
    ...
    return (
        <div>
            <h2>{title}</h2>
            <input type="range" id="slider" min={min} max={max} step="0.5" defaultValue={max / 2} onChange={(event) => {
                updateValue(event, "value")
                updateMarkers()
            }}/>
            <input type="number" id="value" min={min} max={max} step="0.5" defaultValue={max / 2} onChange={(event) => {
